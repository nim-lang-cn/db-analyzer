...
当连接到数据库执行 `EXPLAIN SELECT ...`（或者 `EXPLAIN ANALYZE SELECT ...`） 的时候，每次返回的结果都会不同。比如第一次返回 0.3 ms，
第二次返回 0.6ms，... 为了保证分析结果的精确，我们需要重复分析多次，然后以最后的区间（平均最低 - 平均最高）作为结果。如果重复查询 1000 次，
大部分结果能保证在 0.3ms ~ 0.6ms 之间，但是有时候会出现 10ms 这种与区间差别非常大的情况，这可能是因为网络、磁盘或者一些其他环境原因造成的。
这样的数据称为 “抖动”。为了保证结果更加稳定，可以删除 1000 次结果中花费时间最常的几个（比如 10 个），可以大幅度减少 “抖动” 的影响。

## 6. 伪数据

在基准测试之前，需要为数据库准备好数据集。除非是一个运行了很久的商业产品，否则你很难在一开始能够拥有充足的数据。因此，为了测量
大数据集环境的真实性能，需要通过一些方法为数据库制造伪数据。

通常，根据需要可以制造某个规模的数据集。比如单表 10000 行、1000000 行、10000000 行、... 行数越多，在准备数据的时候花费
的时间就越长。

`<tables>` 指定了如何准备以及准备多少行数据集。

还有一个要注意的就是，数据集需要随机性。对于 PostgreSQL 数据库，提供了 `generate_series(from, to)` 函数来自动生成一定数量的
整数。

在 DBAnalyzer 中，当前版本 `<table>.<query>` 可以通过指定 `{i}` 获得程序生成的序列整数，比如从 1 到 10000 （行）。在下一个版本中，
支持 `<table>` 定义 `<arg>` 由用户指定参数的名字和类型（整数或者时间）。
